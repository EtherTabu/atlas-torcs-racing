        ...
        while True:
            ...
            try:
                ...
                if n_fail < 0:
                    self.so.close()
                    self.so = None
                    raise TimeoutError('No TORCS SCR server responded; no simulator was relaunched.')
                n_fail -= 1
...
def run_controller(drive):
    C = Client()  # Preserve the command-line port; default remains 3001.
    try:
        with Telemetry(drive.__name__, live=not C.debug) as telemetry:
            for step in range(1, C.maxSteps + 1):
                C.get_servers_input()
                if not C.so:
                    break  # Shutdown/restart is not a fresh sensor sample.
                drive(C)
                C.respond_to_server()
                telemetry.record(C.S.d, C.R.d, step, C.S.servstr)
...
# Legacy drive_example remains importable for explicitly labelled comparisons.
# The only executable entry point is at the end of this file.
...
STEER_GAIN = 8      # Steering sensitivity. Higher values make the car turn more aggressively.
...
# ================= MAIN LOOP =================
def drive_geometry_v1(c):
    from racing_controller import Driver
    if not hasattr(c, 'geometry_driver'):
        c.geometry_driver = Driver()
    c.R.d.update(c.geometry_driver.step(c.S.d))


def drive_mapped_v2_r1(c):
    import json
    from pathlib import Path
    from mapped_controller import Driver
    if not hasattr(c, 'mapped_driver'):
        params = json.loads(Path(__file__).with_name('best_params.json').read_text())
        c.mapped_driver = Driver(params)
    c.R.d.update(c.mapped_driver.step(c.S.d))


if __name__ == "__main__":
    run_controller(drive_mapped_v2_r1)
