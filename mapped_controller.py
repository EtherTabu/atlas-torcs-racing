...
DEFAULTS = dict(lateral_accel=5.0, braking_accel=4.0, max_speed=170,
                ...
                upshift_rpm=8000, downshift_rpm=3500, traction_slip=0, zones=[])
...
class Driver:
    ...
    def __init__(self, params=None):
        ...
        for _ in range(3):
            for i in range(self.n-1, -1, -1):
                self.envelope[i] = min(self.envelope[i], math.sqrt(self.envelope[(i+1)%self.n]**2 + 2*p['braking_accel']*self.ds))
        self.steer = 0
        self.gear = 1
        self.shift_wait = 0
        self.last_time = None
    ...
    def step(self, s):
        ...
        desired = ((p['feedforward']+p['understeer']*v*v)*k + p['heading_gain']*s['angle']
                   -p['position_gain']*s['trackPos']-p['lateral_damping']*s['speedY'])
        ...
        if now <= 0 or speed < 5:
            ...
            if s['rpm']>p['upshift_rpm'] and self.gear<6:
                self.gear += 1
                self.shift_wait = .7
            elif s['rpm']<p['downshift_rpm'] and self.gear>1:
                self.gear -= 1
                self.shift_wait = .7
